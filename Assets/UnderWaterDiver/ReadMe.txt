...
Before use, Standard Assets must be imported into the project in order to run the demo scenes.
Standard Assets: Image Effects
Standard Assets: Characters.ThirdPerson
Standard Assets: Cross Platform Input
Standard Assets: Environment

After importing those packages, load the ThirdPersonSwimmerController script as well as the CameraUnderwater script. Find all the lines that are marked with an "uncomment" comment.
Once these are uncommented, the package should be able to run.
